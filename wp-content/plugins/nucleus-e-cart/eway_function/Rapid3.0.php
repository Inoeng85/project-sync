...
class RapidAPI {
    ...
    function __construct() {
		...
		if($GLOBALS['EWAY_CONNECTION'] == "Live")
			$this->APIConfig = parse_ini_file("config_live.ini");
		if($GLOBALS['EWAY_CONNECTION'] == "Sandbox")
			$this->APIConfig = parse_ini_file("config_sandbox.ini");
    }
    ...
    public function CreateAccessCodeSOAP($request) {
        ...
        try {
            $client = new SoapClient($this->APIConfig["PaymentService.Soap"], array(
                        'trace' => true,
                        'exceptions' => true,
                        'login' => $GLOBALS['EWAY_API_USERNAME'],
                        'password' => $GLOBALS['EWAY_API_PASSWORD'],
                    ));
    ...
    private function PostToRapidAPI($url, $request, $IsPost = true) {
        ...
        curl_setopt($ch, CURLOPT_USERPWD, $GLOBALS['EWAY_API_USERNAME'] . ":" . $GLOBALS['EWAY_API_PASSWORD']);
        if ($IsPost)
        ...
        if (curl_errno($ch) != CURLE_OK) {
			...
			$GLOBALS['ERROR_EWAY'] = true;
        } else {
			$GLOBALS['ERROR_EWAY'] = false;
            curl_close($ch);
            return $response;
        }
    }
